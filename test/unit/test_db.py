...
class TestDB(unittest.TestCase):
    ...
    def test_listdir(self):
        ...
        w_dir = tempfile.mkdtemp()
        root = os.path.join(w_dir, 'db_root')
        db_ = alder.db.DB(root, hash_limit=0xfffff)
        data = {1:2}
    ...
    def test_json(self):
        ...
        w_dir = tempfile.mkdtemp()
        root = os.path.join(w_dir, 'db_root')
        db_ = alder.db.DB(root, serial='json')
        data = {u'1':2}
        db_.insert('foo', data)
        pull_data = db_.get('foo')
        self.assertEqual(data, pull_data)
    ...
    def test_higherarchy(self):
        ...
        self.assertEqual(
                table_entry['tfn'],
                os.path.join(root, 'foo', 'bar', 'alder_table_0'))
        shutil.rmtree(w_dir)
    ...
    def test_top_loc(self):
        ...
        self.assertEqual(
                table_entry['tfn'],
                os.path.join(root, 'alder_table_0'))
        shutil.rmtree(w_dir)
    ...
    def test_inv_fmt(self):
        ...
        w_dir = tempfile.mkdtemp()
        root = os.path.join(w_dir, 'db_root')
        db_ = alder.db.DB(root, fmt='<KsQH', fmt_map=['key', 'prev', 'rev'])
        data = {1:2}
        db_.insert('foo', data)
        pull_data = db_.get('foo')
        self.assertEqual(data, pull_data)
    ...
    def test_extra_index_data(self):
        ...
        db_ = alder.db.DB(root)
        for _ in range(100):
            data = alder.aid.rand.rand_dict()
            while True:
                try:
                    ind_extra = alder.aid.rand.rand_dict()
